...
import com.emplk.realestatemanager.ui.add.AddPropertyTypeViewStateItem
...
class AddPropertyTypeSpinnerAdapter : ListAdapter, Filterable {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        getItem(position)?.let { item ->  // let function ensures that you only perform the operations inside the block if getItem(position) returns a non-null item.
            binding.addPropertySpinnerItemTvName.text = item.propertyType
        }
        return binding.root
    }
    ...
    override fun getFilter() = object : Filter() {
        ...
        override fun convertResultToString(resultValue: Any): CharSequence =
            (resultValue as AddPropertyTypeViewStateItem).propertyType
    }
